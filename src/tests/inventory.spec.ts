...
test.describe(
  "Inventory",
  {
    tag: ["@inventory", "@regression", "@smoke"],
    annotation: [
      {
        type: "Test Case",
        description: "Inventory Test Cases",
      },
    ],
  },
  () => {
    ...
    test("sort Price low to high shows cheapest item first", async ({
      page,
    }) => {
      const inventoryPage = new InventoryPage(page);
    ...
    test("cart badge increments correctly as items are added one by one", async ({
      ...
      await expect
        ...
        .toHaveText(
          String(
            INVENTORY.TOTAL_ITEMS === count ? INVENTORY.TOTAL_ITEMS : count,
          ),
        );
    });
  },
