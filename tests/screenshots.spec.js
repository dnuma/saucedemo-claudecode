...
test.describe('Login Screenshots', () => {
  ...
  test('standard_user - inventory page after login', async ({ page }) => {
    ...
    await expect(page).toHaveScreenshot('standard-user-inventory.png', { fullPage: true });
  });
  ...
  test('locked_out_user - error message on login page', async ({ page }) => {
    ...
    await expect(page).toHaveScreenshot('locked-out-user-error.png', { fullPage: true });
  });
  ...
  test('problem_user - inventory with broken images', async ({ page }) => {
    ...
    await page.waitForURL(URLS.INVENTORY);
    await expect(page).toHaveScreenshot('problem-user-inventory.png', { fullPage: true });
  });

  // performance_glitch_user: login is artificially delayed — extended timeout before screenshot
  test('performance_glitch_user - inventory after delayed login', async ({ page }) => {
    await loginPage.login(process.env.PERFORMANCE_GLITCH_USER, process.env.PASSWORD);
    await page.waitForURL(URLS.INVENTORY, { timeout: TIMEOUTS.PERFORMANCE_GLITCH });
    await expect(page).toHaveScreenshot('performance-glitch-user-inventory.png', { fullPage: true });
  });
  ...
  test('error_user - inventory page after login', async ({ page }) => {
    ...
    await page.waitForURL(URLS.INVENTORY);
    await expect(page).toHaveScreenshot('error-user-inventory.png', { fullPage: true });
  });
